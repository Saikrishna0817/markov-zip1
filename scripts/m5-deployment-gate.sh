...
for compiler in g++ clang++; do
  ...
  rm -rf "$build"
  cmake -S "$ROOT" -B "$build" -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_CXX_COMPILER="$compiler" -DSIHOPT_WARNINGS_AS_ERRORS=ON
  cmake --build "$build" --parallel 2
  ctest --test-dir "$build" --output-on-failure
  sanitize="$ROOT/_m5-deploy-$tag-sanitize"
  rm -rf "$sanitize"
  cmake -S "$ROOT" -B "$sanitize" -DCMAKE_BUILD_TYPE=Debug -DCMAKE_CXX_COMPILER="$compiler" -DSIHOPT_ENABLE_ASAN_UBSAN=ON
...
rm -rf "$fuzz"
cmake -S "$ROOT" -B "$fuzz" -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_CXX_COMPILER=clang++ -DSIHOPT_BUILD_FUZZER=ON
