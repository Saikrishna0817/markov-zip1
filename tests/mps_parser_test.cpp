...
int main() {
    ...
    int count = 0;
    markov_cero::test_support::enumerate_integer_box({0, 1}, {2, 2},
                                                     [&](const std::vector<int>&) { ++count; });
    require(count == 6, "box enumeration");
    ...
    bool rejected = false;
    try {
        (void)markov_cero::io::parse_mps_string("NAME X\nROWS\n N O\nQSECTION\nENDATA\n");
    } catch (const markov_cero::io::MpsError&) {
        rejected = true;
    }
    require(rejected, "unknown section");
    ...
    const auto integer = markov_cero::io::parse_mps_string(
        "NAME I\nROWS\n N O\n L R\nCOLUMNS\n M 'MARKER' 'INTORG'\n X O 1 R 1\n M2 'MARKER' "
        "'INTEND'\nRHS\n R1 R 1\nENDATA\n");
    require(integer.variable_type[0] == markov_cero::model::VariableType::integer,
            "integer marker");
    rejected = false;
    try {
        (void)markov_cero::io::parse_mps_string("NAME X\nROWS\n N O\nENDATA\nROWS\n");
    ...
    require(rejected, "non-ASCII name");
    rejected = false;
    try {
        (void)markov_cero::io::parse_mps_string("NAME X\nROWS junk\n N O\nENDATA\n");
    } catch (const markov_cero::io::MpsError&) {
    ...
    require(rejected, "section header arity");
    rejected = false;
    try {
        (void)markov_cero::io::parse_mps_string(
            "NAME X\nROWS\n N O\n L R\nCOLUMNS\n X O 1 R 1\nBOUNDS\n BV B X\n FR B X\nENDATA\n");
    ...
    require(rejected, "binary bounds remain within zero and one");
    const int old_round = std::fegetround();
    std::fesetround(FE_UPWARD);
    markov_cero::model::Model binary;
    binary.name = "B";
    binary.matrix = markov_cero::model::SparseMatrixBuilder(0, 1).build();
    binary.variable_name = {"x"};
    binary.objective = {1};
    binary.variable_lower = {markov_cero::model::Bound::finite(0)};
    binary.variable_upper = {markov_cero::model::Bound::finite(1)};
    binary.variable_type = {markov_cero::model::VariableType::binary};
    binary.validate();
    const auto rounded =
        markov_cero::verify::verify_primal(binary, {{1.2}, 1.2}, {0, 0}, {0, 0}, 0.0);
    require(std::abs(rounded.maximum_integrality_violation - 0.2) < 1e-12,
            "rounding-mode independent integrality");
    std::fesetround(old_round);
    const auto tiny = markov_cero::io::parse_mps_string(
        "NAME T\nROWS\n N O\n E BAND\nCOLUMNS\n X O 1 BAND 1\n Y O 2 BAND 1\nRHS\n R BAND "
        "2\nRANGES\n G BAND 1\nBOUNDS\n UI B X 2\n UI B Y 2\nENDATA\n");
    int feasible_count = 0, infeasible_count = 0;
    markov_cero::test_support::enumerate_integer_box(
        {0, 0}, {2, 2}, [&](const std::vector<int>& point) {
            const Rational activity = Rational(point[0]) + Rational(point[1]);
            const Rational objective = Rational(point[0]) + Rational(2) * Rational(point[1]);
            const bool exact_feasible = Rational(2) <= activity && activity <= Rational(3);
            const auto checked = markov_cero::verify::verify_primal(
                tiny,
                {{static_cast<double>(point[0]), static_cast<double>(point[1])},
                 static_cast<double>(objective.numerator()) / objective.denominator()},
                {0, 0}, {0, 0}, 0.0);
            require(checked.passed == exact_feasible,
                    "exact exhaustive oracle/verifier disagreement");
            if (exact_feasible)
                ++feasible_count;
            else
                ++infeasible_count;
