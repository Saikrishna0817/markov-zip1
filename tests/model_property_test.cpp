    ...
    for (int trial = 0; trial < 250; ++trial) {
        ...
        for (std::size_t column = 0; column < columns; ++column)
            for (std::size_t row = 0; row < rows; ++row) {
                if ((random() & 3U) == 0U)
                    continue;
                const double entry = coefficient(random);
                builder.add(row, column, entry);
                dense[row][column] += entry;
                if ((random() & 7U) == 0U) {
                    builder.add(row, column, -entry);
                    dense[row][column] -= entry;
                }
        ...
        for (std::size_t row = 0; row < rows; ++row) {
            ...
            for (std::size_t column = 0; column < columns; ++column)
                expected += dense[row][column] * x[column];
            require(std::abs(actual[row] - static_cast<double>(expected)) <=
                        1e-12 * std::max(1.0, std::abs(actual[row])),
                    "CSC/dense mismatch");
        }
    }
    return 0;
