  ...
  public:
    Rational(std::int64_t numerator = 0, std::int64_t denominator = 1) {
        ...
        if (numerator == std::numeric_limits<std::int64_t>::min() ||
            ...
            throw std::overflow_error("tiny rational input outside safe range");
        if (numerator < -1000000 || numerator > 1000000 || denominator < -1000000 ||
            denominator > 1000000)
            throw std::overflow_error("tiny rational input outside safe range");
        if (denominator < 0) {
            numerator = -numerator;
...
inline void enumerate_integer_box(const std::vector<int>& lower, const std::vector<int>& upper,
    ...
    while (true) {
        ...
        while (position > 0U && point[position - 1U] == upper[position - 1U])
            --position;
        if (position == 0U)
            return;
        --position;
        ++point[position];
        for (std::size_t j = position + 1U; j < point.size(); ++j)
            point[j] = lower[j];
    }
}
} // namespace markov_cero::test_support
