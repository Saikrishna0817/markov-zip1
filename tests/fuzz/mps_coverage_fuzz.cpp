...
namespace { std::array<unsigned char, 65536> trial{}, total{}; }
...
int main(int argc, char** argv) {
    ...
    while(std::chrono::steady_clock::now()<deadline){
        ...
        if(op==0U && !input.empty()) input[random()%input.size()]^=static_cast<char>(1U<<(random()%8U));
    ...
    std::size_t edges=0; for(auto value:total) edges+=value!=0U;
    std::cout<<"coverage-guided fuzz passed: executions="<<executions<<" corpus="<<corpus.size()<<" discoveries="<<discoveries<<" edge_slots="<<edges<<" seconds="<<seconds<<"\n";
}
