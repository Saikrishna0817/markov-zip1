...
double compute_strong_branching_score(double delta_down, double delta_up, double mu) noexcept {
...
StrongBranchingResult evaluate_strong_branching(
    ...
    if (candidate_indices.empty()) {
        eval_indices =
            find_fractional_variables(primal, model.variable_type, options.integrality_tolerance);
    } else {
    ...
    if (options.max_candidates > 0 && eval_indices.size() > options.max_candidates) {
        std::stable_sort(eval_indices.begin(), eval_indices.end(),
    ...
    for (std::size_t var_idx : eval_indices) {
        ...
        if (model.variable_lower[var_idx].is_finite() &&
            ...
            try {
                const auto canon_down =
                    transform::sparse_canonicalize(down_model, /*relax_integrality=*/true);
                ...
                if (current_basis.has_value() && current_basis->rows == dense_down.matrix.rows &&
                    ...
                    try {
                        child_warm =
                            lp::dual::make_basis_state(dense_down, current_basis->basic_variables);
                    } catch (...) {
        ...
        if (model.variable_upper[var_idx].is_finite() &&
            ...
            try {
                const auto canon_up =
                    transform::sparse_canonicalize(up_model, /*relax_integrality=*/true);
                ...
                if (current_basis.has_value() && current_basis->rows == dense_up.matrix.rows &&
                    ...
                    try {
                        child_warm =
                            lp::dual::make_basis_state(dense_up, current_basis->basic_variables);
                    } catch (...) {
                        child_warm = std::nullopt;
                    }
                }
...
StrongBranchingResult evaluate_strong_branching(
    const model::Model& model, const std::vector<double>& primal, double current_obj,
    const std::optional<lp::dual::BasisState>& current_basis, const StrongBranchingOptions& options,
    std::vector<VariablePseudoCost>* pseudo_costs) {
    return evaluate_strong_branching(model, primal, current_obj, current_basis, {}, options,
                                     pseudo_costs);
}
