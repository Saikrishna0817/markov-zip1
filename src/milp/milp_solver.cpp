...
Result solve(const model::Model& model, const Options& options) {
    ...
    while (!queue.empty() && result.nodes_explored < options.max_nodes) {
        ...
        if (node->id == 0) {
            ...
            if (node_lp_res.status == lp::reference::SolveStatus::infeasible) {
                continue;  // proven infeasible child
            }
            if (node_lp_res.status != lp::reference::SolveStatus::optimal) {
                // Numerical / iteration / resource failure — not a proof of prune
                unresolved_lp = true;
                continue;
            }
    ...
    const bool complete = !search_incomplete && !unresolved_lp && queue.empty();
    ...
    if (!best_primal.empty()) {
        ...
        if (complete) {
            ...
            // Incumbent exists but search stopped early or left unresolved LPs
            result.status = lp::reference::SolveStatus::resource_limit;
            result.message = "incomplete search – feasible incumbent returned";
        }
    } else {
        if (complete) {
            result.status = lp::reference::SolveStatus::infeasible;
            ...
            result.status = lp::reference::SolveStatus::resource_limit;
            result.message = "incomplete search – no incumbent";
        }
    }
    return result;
}
