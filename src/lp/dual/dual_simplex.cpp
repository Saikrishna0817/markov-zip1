...
linalg::SparseCsc sparse_basis_matrix(const transform::CanonicalModel& m, const std::vector<std::size_t>& basis) {
...
std::vector<double> full_primal(std::size_t n, const std::vector<std::size_t>& basis, const std::vector<double>& xb) {
...
void validate_options(const Options& o) {
    if (!o.iteration_limit || o.iteration_limit > maximum_iterations || o.telemetry_limit > maximum_telemetry ||
        !std::isfinite(o.feasibility_tolerance) || !std::isfinite(o.dual_tolerance) ||
        !std::isfinite(o.pivot_tolerance) || !std::isfinite(o.condition_trigger) || o.feasibility_tolerance <= 0 ||
...
void validate_basis(const transform::CanonicalModel& m, const BasisState& s) {
    if (s.rows != m.matrix.rows || s.columns != m.matrix.columns || s.model_fingerprint != fingerprint(m) ||
        s.basic_variables.size() != m.matrix.rows) {
...
Result cold(const transform::CanonicalModel& m, const Options& o, const std::string& why) {
    ...
    out.message = why;
    if (out.solution.status == reference::SolveStatus::optimal && out.solution.basis.size() == m.matrix.rows) {
        out.basis_state = make_basis_state(m, out.solution.basis);
    }
    return out;
}

std::size_t select_leaving_row(const transform::CanonicalModel& m, linalg::SparseBasisFactorization& factor,
    ...
    for (std::size_t i = 0; i < m.matrix.rows; ++i) {
        ...
        const double score = (-xb[i]) / std::sqrt(std::max(weight, std::numeric_limits<double>::min()));
        if (leaving == m.matrix.rows || score > worst || (score == worst && basis[i] < basis[leaving])) {
            leaving = i;
            worst = score;
        }
    }
...
std::size_t select_entering_column(const transform::CanonicalModel& m, const std::vector<double>& alpha,
                                   const std::vector<double>& rc, const std::vector<bool>& is_basic,
                                   const Options& o, double& best_ratio) {
    ...
    for (std::size_t j = 0; j < m.matrix.columns; ++j) {
        ...
        if (o.harris_ratio) {
            if (q <= limit &&
                (entering == m.matrix.columns || -alpha[j] > best_pivot || (-alpha[j] == best_pivot && j < entering))) {
                entering = j;
                best_ratio = q;
                best_pivot = -alpha[j];
            }
...
Result certified_optimal(const transform::CanonicalModel& m, const std::vector<std::size_t>& basis,
                         const std::vector<double>& xb, const std::vector<double>& y, const Options& o) {
    Result out;
    ...
    auto check = verify::verify_reference_result(m, out.solution, std::max(o.feasibility_tolerance, o.dual_tolerance));
    if (!check.accepted) {
...
Result certified_farkas(const transform::CanonicalModel& m, const std::vector<double>& pi, const Options& o) {
    Result out;
...
Result solve(const transform::CanonicalModel& m, const Options& o, const std::optional<BasisState>& warm) {
    Result out;
    ...
    try {
        ...
        auto factor = linalg::SparseBasisFactorization::factorize(sparse_basis_matrix(m, basis), sparse_options(o));
        ...
        for (std::size_t step = 0; step < o.iteration_limit; ++step) {
            ...
            if (m.matrix.rows > 0 && diagnostics.maximum_absolute_pivot > 0 &&
                diagnostics.minimum_absolute_pivot / diagnostics.maximum_absolute_pivot < o.condition_trigger) {
            ...
            double best_ratio = 0;
            const std::size_t entering = select_entering_column(m, alpha, rc, is_basic, o, best_ratio);
            ...
            if (out.telemetry.size() < o.telemetry_limit) {
                out.telemetry.push_back({step, dot(cb, xb) + m.objective_offset, xb[leaving], basis[leaving], entering,
                                         alpha[entering], o.harris_ratio});
            } else {
