...
Numerical perturbation is not enabled in the dense reference path; deterministic tie-breaking and full refactorization are used instead, so perturbation cleanup count remains zero. Any future perturbation must restore the original problem and reverify the result.

Deployment CMake/Clang remains deferred to M5. M5 must not start without explicit approval.
